...
def main():
    ...
    if hdr.boot_magic == hdr.MAGIC.encode('UTF-8'):
        if verb:
            print( "boot_magic correct: ", hdr.boot_magic)
    else:
        print( "boot_magic not correct: ", hdr.boot_magic, "  expecting:", hdr.MAGIC)
        print(str(hdr.boot_magic))
        in_file.close()
        sys.exit(1)
    ...
    # print( "product name: ", hdr.product_name)
    print( "product name: ", hdr.product_name.rstrip(b'\x00').decode('utf-8'))

    # Read cmdline.
    hdr.cmdline = in_file.read(hdr.ARGS_SIZE, "cmdline")
    # print( "cmdline: ", hdr.cmdline)
    print( "cmdline: ", hdr.cmdline.rstrip(b'\x00').decode('utf-8'))
